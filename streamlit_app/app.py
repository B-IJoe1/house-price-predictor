...
st.title("House Market Price Predictor")
st.markdown(
    """
...
with col1:
    ...
    with bed_col:
        ...
        bedrooms = st.selectbox("Bedrooms", options=[1, 2, 3, 4, 5, 6], index=2, label_visibility="collapsed")

    with bath_col:
        st.markdown("<p><strong>Bathrooms</strong></p>", unsafe_allow_html=True)
        bathrooms = st.selectbox("Bathrooms", options=[1, 1.5, 2, 2.5, 3, 3.5, 4], index=2, label_visibility="collapsed")
...
version = os.getenv("APP_VERSION", "4.1.0")  # Default version if not set in environment
